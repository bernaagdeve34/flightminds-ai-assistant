    ...
    <div className="card-surface overflow-hidden">
      <div className="px-4 pt-3">
        <div className="flex items-center justify-between">
          ...
          <div className="w-44 h-2 rounded-full bg-white/70 relative overflow-hidden">
            <div className="absolute inset-y-0 left-0 w-1/2 rounded-full ist-bar"></div>
          </div>
        </div>
        <div className="mt-1 text-xs font-medium text-gray-600">{dateLabel}</div>
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-white text-left border-y border-gray-100">
              <th className="px-3 py-2 font-semibold" style={{color:"var(--ist-teal)"}}>{language === "tr" ? "Tarih" : "Date"}</th>
              <th className="px-3 py-2 font-semibold" style={{color:"var(--ist-teal)"}}>{language === "tr" ? "Planlanan / Tahmini" : "Scheduled / Estimated"}</th>
              <th className="px-3 py-2 font-semibold" style={{color:"var(--ist-teal)"}}>{language === "tr" ? "Hava Yolu" : "Airline"}</th>
              <th className="px-3 py-2 font-semibold" style={{color:"var(--ist-teal)"}}>{language === "tr" ? "Uçuş No" : "Flight No"}</th>
              <th className="px-3 py-2 font-semibold" style={{color:"var(--ist-teal)"}}>{language === "tr" ? "Kalkış / Varış" : "Origin / Dest"}</th>
              <th className="px-3 py-2 font-semibold" style={{color:"var(--ist-teal)"}}>{tableDirection === "Arrival" ? (language === "tr" ? "Bagaj" : "Baggage") : (language === "tr" ? "Kapı" : "Gate")}</th>
              <th className="px-3 py-2 font-semibold" style={{color:"var(--ist-teal)"}}>{t.headers.status}</th>
            </tr>
          </thead>
          <tbody>
            {flights.map((f) => (
              <tr key={f.id} className="border-t border-gray-100 hover:bg-gray-50/50">
                <td className="px-3 py-2 whitespace-nowrap text-gray-700">{new Date(f.scheduledTimeLocal).toLocaleDateString(language === "tr" ? "tr-TR" : "en-US", { year: "numeric", month: "long", day: "numeric", weekday: "long" })}</td>
                ...
                <td className="px-3 py-2 text-gray-700">{f.airline}</td>
                <td className="px-3 py-2 font-semibold text-gray-900">{f.flightNumber}</td>
                <td className="px-3 py-2 text-gray-700">{f.direction === "Arrival" ? f.originCity : f.destinationCity}</td>
